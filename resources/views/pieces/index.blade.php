...
@section('content')
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="white-box">
            <h3 class="box-title">Danh sách pieces</h3>
                      ...
                      @if ($pieces)
                        ...
                        @foreach($pieces as $piece)
                          <tr>
                              <td>{{ $i }}</td>
                              <td>
                                <img style="width: 100px;" src="{{ $piece->urlPath('100x100') }}" /><br/>
                              </td>
                              <td>
                                {{ $piece->title }}
                                <br/>
                                <a href="/piece/{{ $piece->slug }}" target="_blank">{{ $piece->slug }}</a>
                              </td>
                              <td>{{ $piece->displayStatus() }}</td>
                              <td><a class="btn btn-info btn-margin" href="/pieces/{{$piece->id}}/edit" ><i class="fa fa-pencil"></i></a>
                                  <button data-toggle="modal" data-target="#mod-error" class="delete_equipment btn btn-danger btn-margin" pieces-id="{{ $piece->_id }}" ><i class="fa fa-times"></i></button>
                               </td>
                          </tr>
                          <?php $i++?>
                        @endforeach
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        ...
        $('.delete_equipment').click(function() {
          ...
          $('#form_model').attr('action', '/pieces/'+id+'/destroy');
        });
